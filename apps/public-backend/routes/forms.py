...
<p>The requested form could not be found or is not currently published.</p>
<p><a href="{home}">Return to all forms</a></p>
</main>
</body>
</html>
"""
...
def get_form_og(
    ...
    try:
        ...
        body_404 = _OG_404_TEMPLATE.format(
            home=html.escape(origin + "/", quote=True),
        ).encode("utf-8")
        return Response(
            content=body_404,
    ...
    body = _OG_TEMPLATE.format(
        ...
        # Inside <script type="application/ld+json"> escape angle brackets
        # so that neither opening (<script>) nor closing (</script>) tags
        # can inject HTML.  \u003c / \u003e are valid in JSON strings.
        jsonld=(
            json.dumps(jsonld_obj, separators=(",", ":"))
            .replace("<", r"\u003c")
            .replace(">", r"\u003e")
        ),
    ).encode("utf-8")
