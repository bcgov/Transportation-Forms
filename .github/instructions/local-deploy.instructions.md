---
description: "Use when editing Taskfile, Helm local values, Ingress, Crunchy overrides, or any local deployment files under deploy/local/. Covers Taskfile conventions, local Helm patterns, and k3s compatibility."
applyTo: "deploy/local/**"
---

# Local Deployment File Guidelines

## Taskfile conventions (deploy/local/Taskfile.yml)
...
- Variables: `TAG=local`, `RELEASE_NAME=transportation-forms-local`, `CHART=charts/app`

## Helm local values (charts/app/values-local.yaml)
...
## Ingress (deploy/local/ingress.yaml)
...
## Crunchy overrides (deploy/local/crunchy-overrides.yaml)
...
## Post-renderer (deploy/local/patch-openshift.sh)
